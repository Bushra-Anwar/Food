...
const oldSearchLogicRegex = /\/\/ SEARCH AUTO-SUGGESTIONS LOGIC[\s\S]*?dropdown\.innerHTML = html;\s*container\.classList\.add\('active'\);\s*\}/;

const newSearchLogic = `// SEARCH AUTO-SUGGESTIONS LOGIC
        const productsDB = [
...
files.forEach(file => {
    ...
    if (oldSearchLogicRegex.test(content)) {
        const newContent = content.replace(oldSearchLogicRegex, newSearchLogic);
        fs.writeFileSync(file, newContent, 'utf8');
        modifiedCount++;
        console.log("Updated " + file);
    } else {
        console.log("Skipped " + file + " - could not find old search logic");
    }
});
